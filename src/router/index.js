...
router.beforeEach((to, from, next) => {
  ...
  if (requiresAuth) {
    if (isAuthChecked) {
      if (isAuthenticated) {
        if (requiresAdmin && userRole !== 'admin') {
          ...
          next('/dashboard'); // Redirect non-admin users to the dashboard
        } else {
          next(); // Allow access
        }
      } else {
      ...
      const unwatch = onAuthStateChanged(auth, async (user) => {
        ...
        if (isAuthenticated) {
          if (requiresAdmin) {
            ...
            if (userRole === 'admin') {
              next(); // Allow admin user
            } else {
              alert('Access denied: You do not have permission to access the admin page.');
              next('/dashboard'); // Redirect non-admin users
            }
          } else {
            next(); // Allow access if no admin requirement
          }
        } else {
          next('/login'); // Redirect to login if not authenticated
        }
    ...
    next(); // Proceed normally if route doesn't require auth
  }
});
